...
public class MscDetails extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MscDetails window = new MscDetails();
	...
	private void initialize() {
		...
		JLabel lblMscDetails = new JLabel("MSC DETAILS");
		lblMscDetails.setForeground(Color.RED);
		lblMscDetails.setFont(new Font("Cambria", Font.BOLD, 55));
		lblMscDetails.setBounds(162, 74, 403, 99);
		panel.add(lblMscDetails);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ResultSet rs=stmt.executeQuery("Select * from Msc where Id='"+textField.getText()+"'");
		...
		try {
			...
			Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:orcl","hr","msc");
			Statement stmt=con.createStatement();
			ResultSet rs=stmt.executeQuery("Select count(Id) from Msc");
			while(rs.next())
			{
		...
		btnNewButton_1.setBounds(1156, 619, 67, 23);
		...
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:orcl","hr","msc");
					Statement stmt=con.createStatement();
					ResultSet rs=stmt.executeQuery("delete from Msc where Id='"+textField.getText()+"'");
					if(rs.next())
					{
		...
		btnNewButton_2.setBounds(1043, 619, 89, 23);
